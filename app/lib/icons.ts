...
export interface TechIcon {
  ...
  customUrl?: string; // Custom icon URL (overrides Simple Icons)
}
...
export function getIconUrl(slug: string, color: string): string {
  // Check if icon has customUrl
  const icon = Object.values(techIcons).find((i) => i.slug === slug);
  if (icon?.customUrl) {
    return icon.customUrl;
  }
